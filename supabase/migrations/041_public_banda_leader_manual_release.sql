-- Mantem o lider publico conectado ate desconexao manual.
-- O heartbeat deixa de ser necessario para preservar a lideranca.
...
begin
  ...
  set
    ...
    item_type = case
      when public_invite_banda_state.leader_client_id is null then null
    ...
    current_song_index = case
      when public_invite_banda_state.leader_client_id is null then 0
    ...
    is_stage_active = case
      when public_invite_banda_state.leader_client_id is null then false
      else public_invite_banda_state.is_stage_active
    end,
    updated_at = case
      when public_invite_banda_state.leader_client_id is null then now()
  ...
  return jsonb_build_object(
    ...
    'leader', jsonb_build_object(
      'active', v_state.leader_client_id is not null,
